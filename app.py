...
def passenger():
    if request.method == 'POST':
        ...
        if request.form['pass'] == 'Search':
            passDetails = dict((key, request.form.getlist(key)) for key in request.form.keys())
            # pid = passDetails['pid']
            # pmno = passDetails['pmno']
            pfn = '%'+passDetails['pfn'][0]+'%'
            pln = '%'+passDetails['pln'][0]+'%'
            # pdob = passDetails['pdob']
            # psex = passDetails['psex']
            # pidt = passDetails['pidt']
            # pidn = passDetails['pidn']
            cur = mysql.connection.cursor()
            resultValue = cur.execute("SELECT * FROM passenger where pfirst_name like %s and plast_name like %s", (pfn, pln))
            print(resultValue, pfn, pln)
            if resultValue > 0:
                res = cur.fetchall()
                return render_template('passengerview.html', passDetails=res)
            else:
                return render_template('passengersearch.html')
...
# @app.route('/passsearch/<passdetails>')
# def passsearch(passdetails):
